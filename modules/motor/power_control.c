...
static void MotorSenderGrouping(DJIMotorInstance* motor, CAN_Init_Config_s* config)
{
    ...
    // 检查是否发生id冲突
        ...
        {
            if (dji_motor_instance[i]->motor_can_instance->can_handle == config->can_handle && dji_motor_instance[i]->
                motor_can_instance->rx_id == config->rx_id)
            {
                LOGERROR(
...
void PowerControl()
{
    ...
    for (size_t i = 0; i < idx; ++i) // idx实际上是4个
    {
        ...
        if ((motor_setting->close_loop_type & SPEED_LOOP) && (motor_setting->outer_loop_type & (ANGLE_LOOP |
            SPEED_LOOP)))
        {
